...
%\begin{frame}{What if the bias is not a simple parametric form?}\begin{itemize}
%\item \textbf{Highly adaptive lasso (HAL)} is a maximum likelihood estimator over all (or subset of) \textit{càdlàg} functions (right continuous with left limits) with bounded variation norm (Benkeser and van der Laan 2016).\item We use HAL to data-adaptively learn the working submodel for conditional effect of the study indicator $S$.\item Advantages of HAL: \begin{itemize}\item It is \textbf{flexible} and can approximate almost all functions encountered in practice, without making any smoothness assumptions on the true functional form;\item It \textbf{converges} to the true function at a \textbf{fast} rate $n^{-1/3}(\log n)^{2(d-1)/3}$;\item It is an \textbf{MLE}.\end{itemize}\end{itemize}\end{frame}
...
%\begin{frame}\ \nlOur statistical target parameter is the mean of $Y^a$: $\Psi(P)=E_{P^a}Y^a$, where$\Psi:{\cal M}\rightarrow\openr$. \ \nlThis target parameter only depends on $P$ through $Q=Q(P)$. Therefore, we will also denote the target parameter mapping with $\Psi:{\cal Q}=\{Q(P):P\in {\cal M}\}\rightarrow\openr$ so that $\psi_0=\Psi(Q_0)$.\end{frame}
